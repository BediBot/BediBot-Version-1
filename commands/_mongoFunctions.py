...
def get_all_birthdays_today():
	return list(VerifiedUsers.aggregate([{'$match': {'$expr': {
		'$and': [{'$eq': [{'$dayOfMonth': '$birth_date'}, datetime.date.today().day]},
				 {'$eq': [{'$month': '$birth_date'}, datetime.date.today().month]}, ], }, }}]))


def add_due_date_to_upcoming_due_dates(course, type, title, date):
	UpcomingDueDates.insert_one({'course': course, 'type': type, 'title': title, 'date': date})
