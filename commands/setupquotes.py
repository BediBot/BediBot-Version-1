...
async def setup_quotes(ctx: discord.Message, client: discord.Client):
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupQuotes Reply", "What is the quote reaction emoji name? "
                                                                                      "(Must be a custom emoji, enter the name without the : characters)", "blue"))
        try:
            reaction_emoji_message = await client.wait_for('message', timeout = wait_timeout, check = check)
        except asyncio.TimeoutError:
            await response_message.edit(embed = _embedMessage.create("SetupQuotes Reply", "You took too long to respond.", "red"))
            return
        else:
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupQuotes Reply", "How many approvals should be required to approve a quote. (Minimum of 2)", "blue"))
        try:
