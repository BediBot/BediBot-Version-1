...
async def setbirthday(ctx):
	if not _mongoFunctions.is_user_id_linked_to_verified_user(ctx.author.id):
	...
	if error_check == 1:
		await ctx.channel.send(embed = _embedMessage.create("SetBirthday Reply",
															"The syntax is invalid! Make sure it is in the format YYYY MM DD\nEx: $setbirthday 2002 01 01",
															"blue"))
		return
	if error_check == 2:
		await ctx.channel.send(embed = _embedMessage.create("SetBirthday Reply",
