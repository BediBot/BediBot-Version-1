...
async def setup_verification(ctx: discord.Message, client: discord.Client):
    ...
    stop_embed = _embedMessage.create("SetupVerification Reply", "Setup Stopped", "green")

    # Checks if user is admin or bot owner
    if not (ctx.author.guild_permissions.administrator or _util.author_is_bot_owner(ctx)):
        await ctx.channel.send(embed = _embedMessage.create("SetupVerification Reply", "Invalid Permissions", "red"))
        return
    ...
    response_message = await ctx.channel.send(embed = _embedMessage.create("SetupVerification Reply", "Should Verification be Enabled (y/n)?", "blue"))

    while True:
        await response_message.edit(embed = _embedMessage.create("SetupVerification Reply", "Should Verification be Enabled (y/n)?", "blue"))
        try:
            verification_message = await client.wait_for('message', timeout = wait_timeout, check = check)
        except asyncio.TimeoutError:
            await response_message.edit(embed = _embedMessage.create("SetupVerification Reply", "You took too long to respond.", "red"))
            return
        else:
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupVerification Reply", "What is the verified role?", "blue"))
        try:
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupVerification Reply", "What is the verification email domain? (E.g. @uwaterloo.ca)", "blue"))
        try:
    ...
    await ctx.channel.send(embed = _embedMessage.create("SetupVerification Reply", "Verification Setup has been Completed", "blue"))
