...
async def setup_birthdays(ctx: discord.Message, client: discord.Client):
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupBirthdays Reply", "Should Birthday Announcements be Enabled (y/n)?", "blue"))
        try:
            birthday_announcements_message = await client.wait_for('message', timeout = wait_timeout, check = check)
        except asyncio.TimeoutError:
            await response_message.edit(embed = _embedMessage.create("SetupBirthdays Reply", "You took too long to respond.", "red"))
            return
        else:
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupBirthdays Reply", "What is the birthday announcement channel?", "blue"))
        try:
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupBirthdays Reply", "What is the birthday announcement time (HH:MM)? Most would do 00:00 here.", "blue"))
        try:
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupBirthdays Reply",
