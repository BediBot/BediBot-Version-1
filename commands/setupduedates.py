...
async def setup_due_dates(ctx: discord.Message, client: discord.Client):
    ...
    if not (ctx.author.guild_permissions.administrator or _util.author_is_bot_owner(ctx)):
        await ctx.channel.send(embed = _embedMessage.create("SetupDueDates Reply", "Invalid Permissions", "red"))
        return
    ...
    response_message = await ctx.channel.send(embed = _embedMessage.create("SetupDueDates Reply", "Should Due Dates be Enabled (y/n)?", "blue"))

    while True:
        await response_message.edit(embed = _embedMessage.create("SetupDueDates Reply", "Should Due Dates be Enabled (y/n)?", "blue"))
        try:
            due_dates_message = await client.wait_for('message', timeout = wait_timeout, check = check)
        except asyncio.TimeoutError:
            await response_message.edit(embed = _embedMessage.create("SetupDueDates Reply", "You took too long to respond.", "red"))
            return
        else:
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupDueDates Reply", "Which streams require due dates? (Must be integers separated by spaces."
                                                                                        "E.g. 4 8)", "blue"))
        try:
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupDueDates Reply", "What are the term's courses. (Or other deadline categories). "
    ...
    while True:
        await response_message.edit(embed = _embedMessage.create("SetupDueDates Reply", "What are the due date types. "
                                                                                        "(Must be strings separated by spaces. "
    ...
    await ctx.channel.send(embed = _embedMessage.create("SetupDueDates Reply", "DueDate Setup has been Completed", "blue"))
